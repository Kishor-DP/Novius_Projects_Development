...
class TemperatureAnalysis:
    ...
    def calculate_temperatures(self, data):
        ...
        for row in data:
            t1, t2 = row[3], row[4]
            total_temp = t1 + t2
            calculations.append((row[0], row[1], total_temp, total_temp, total_temp, row[2], row[5]))
            print(f"TrainId: {row[0]}, Axle_No: {row[1]}, Total Temperature: {total_temp}, Timestamp: {row[2]}, System_Timestamp: {row[5]}")
        self.db.insert_temperature_calculations(calculations)
        return calculations
...
if __name__ == "__main__":
    ...
    db.monitor_train(train_id, analysis, interval=5, analysis_interval=60)  # Check every 5 seconds and analyze every 60 seconds
