    ...
    try:
        while True:
            try:
                ...
                received_trainid=""#receiver.receive_data()
                logger.info(f"Train_Start_Value: {received_trainid}")
                # Monitor train data and update the database
                ...
                # Start monitoring train data if train_start_value is True
                # if train_start_value:
                #     logger.info("Train Start Value is True. Starting to monitor train data.")
                #     # Monitor train data and update the database
                #     monitor_train_data(db, analysis, receiver)
                 # Start monitoring train data if train_start_value is True
                if train_start_value and not monitoring:
                    logger.info("Train Start Value is True. Starting to monitor train data.")
                    monitoring = True
                    monitor_train_data(db, analysis, receiver)
                elif not train_start_value:
                    if monitoring:
                        logger.info("Train Start Value is False. Stopping monitoring.")
                        monitoring = False
                        # You might want to include a way to cleanly exit the monitor_train_data loop here
                    else:
                        logger.info("Waiting for Train Start Value to become True.")
