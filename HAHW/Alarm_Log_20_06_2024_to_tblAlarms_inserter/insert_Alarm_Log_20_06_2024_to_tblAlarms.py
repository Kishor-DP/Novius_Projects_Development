    ...
    def load_and_filter_settings(input_file, settings_json):
        try:
            ...
            for record in filtered_records:
                ...
                AlarmTempratureSetting = record['decAlarmTemprature']
                WarningTempratureSetting = record['decWarningTemprature']
                print(f"Alarm Temperature Setting: {AlarmTempratureSetting}")
                print(f"Warning Temperature Setting: {WarningTempratureSetting}")
                GlobalVariableHolder.set_global_variable(AlarmTempratureSetting)
...
class fetch_from_Alarm_Log_20_06_2024_insert_to_tblAlarms:
    ...
    def insert_Alarm_Log_20_06_2024_into_tblAlarms(self, json_file):
        ...
        for record in data:
            ...
            self.cursor.execute(check_query, (record.get("intTempLogId"),))
            exists = self.cursor.fetchone()[0]
            if exists:
                print(f"Record with intTempLogId {record.get('intTempLogId')} already exists. Skipping insertion.")
                continue

            filtering_records_for_tblAlarms_insert=[
                record.get("intTempLogId"),
                record.get("intTrainId"),
                record.get("intBogiTypeId"),
            ...
            decTemperature = max(filter(None, temperature_fields))
            #time.sleep(30)
            #print("decTemperature:",decTemperature)
            query = """
            ...
            try:
                values = (
                    ...
                    record["intAxleNo"],
                    record["NvcharCoachNo"],
                    record["IntCoachPosition"],
                    
                    decTemperature,
                    
                    
                    
                    record["nvcharDescription"],
                    record["dtDateofCreation"],
                    record["dtDateofModification"],
                    record["ynDeleted"]
                )
                ...
                self.connection.commit()
                print(f"Records from {json_file} inserted into tblAlarms")
            except KeyError as e:
