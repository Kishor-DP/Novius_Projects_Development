...
def insert_into_ICF_Detected(conn,Time_stamp,Code,Component,Parameter,Status,):
    try:
        ...
        cursor = conn.cursor()
        insert_query = '''
            INSERT INTO MVIS_left (Time_stamp,Code,Component,Parameter,Status)
            VALUES (%s, %s, %s, %s, %s)
        '''
        ...
        app_event.debug("data_ inserted: %s", record_values)
...
def main():
    ...
    #insert_into_ICF_Detected(conn, "test1", "test1","test1", "test1","ok")
