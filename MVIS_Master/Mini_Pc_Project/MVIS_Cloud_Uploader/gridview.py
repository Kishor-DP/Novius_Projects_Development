...
def fetch_data_by_Code(conn, Code):
    # Fetch data from the database (tblTemperatureLog table) based on IntTrainId
    cursor = conn.cursor()
    select_query = '''
        SELECT Sr_No, Code, Time_Stamp_Gridviewtbl
        FROM [MVISCopy].[dbo].[Gridviewtbl]
        WHERE Code = ?
    '''
...
def insert_into_cloud_temp_log(conn, data):
    ...
    cursor = conn.cursor()
    insert_query = '''
        INSERT INTO Gridviewtbl(Sr_No, Code, Time_Stamp_Gridviewtbl)
        VALUES (?,?,?)
    '''
...
def compare_and_sync_databases(config, Code):
    ...
    # Fetch data for the specified Code from the local database
    data_to_sync = fetch_data_by_Code(conn_local, Code)
    ...
    if data_to_sync:
        ...
        print(f"Inserted data into cloud database for Code: {Code}:")
        for entry in inserted_data:
            print(entry)
    else:
        print(f"No data found in the local database for Code: {Code}")
...
def main():
    ...
    Code_from_trigger = config1["Code"]
    
    compare_and_sync_databases(config, Code_from_trigger)
